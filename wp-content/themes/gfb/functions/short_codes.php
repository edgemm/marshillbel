...
function tmk_google_map( $atts, $content = null ) {
    extract(shortcode_atts(array(
		'latitude' => '40.72',
		'longitude' => '-74',
		'zoom' => '12',
    ...
    $output.='<div class="map" id="map_canvas"></div><input type="hidden" class="map_latitude" value="'.$latitude.'" /><input type="hidden" class="map_longitude" value="'.$longitude.'" /><input type="hidden" class="map_zoom" value="'.$zoom.'" /><input type="hidden" class="map_marker" value="'.$marker.'" />';
    return $output;
}

add_shortcode('map', 'tmk_google_map');
...
<?php if ($loop->have_posts()): ?>
	...
	?>
			<?php 
			$post_item='post-item';
			if ($count==1)  $post_item='post-item-first'; ?>
			<article class="<?php echo $post_item ?> clearfix">
				...
				<a class="single-img" href="<?php the_permalink(); ?>">
